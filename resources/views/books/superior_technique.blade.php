...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-brand-50">
                <h2 class="font-heading font-bold text-base text-brand-800">Supérieur Général</h2>
                <p class="text-xs text-brand-600 mt-0.5">Formations académiques universitaires</p>
            </div>
            <div class="p-6 flex flex-col gap-3">
                @foreach([
                    'licence'  => ['Licence',  'Bac+3', 'Fondations académiques'],
                    'master'   => ['Master',   'Bac+5', 'Approfondissement et spécialisation'],
                    'doctorat' => ['Doctorat', 'Bac+8', 'Recherche et innovation scientifique'],
                ] as $level => [$label, $badge, $desc])
                    <a href="{{ route('superior.general', ['level' => $level]) }}"
                       class="group flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:border-brand-300 hover:bg-brand-50 transition-colors">
                        <div>
                            <div class="flex items-center gap-2 mb-0.5">
                                <span class="font-semibold text-sm text-ink group-hover:text-brand-800">{{ $label }}</span>
                                <span class="text-xs px-2 py-0.5 rounded-full bg-brand-100 text-brand-700 font-medium">{{ $badge }}</span>
                            </div>
                            <p class="text-xs text-ink-muted">{{ $desc }}</p>
                        </div>
                        <svg class="w-5 h-5 text-ink-faint group-hover:text-brand-600 shrink-0 transition-colors"
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-emerald-50">
                <h2 class="font-heading font-bold text-base text-emerald-800">Supérieur Technique</h2>
                <p class="text-xs text-emerald-600 mt-0.5">Formations professionnalisantes</p>
            </div>
            <div class="p-6 flex flex-col gap-3">
                @foreach([
