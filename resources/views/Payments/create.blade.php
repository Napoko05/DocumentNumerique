@extends('layouts.app')

@section('title', 'Paiement sécurisé')

@section('content')

<div class="payment-page">

<div class="payment-container">

    {{-- ============================================================
        HEADER
    ============================================================= --}}
    <div class="payment-header">

        <div class="payment-security-icon">
            <i class="bi bi-shield-lock-fill"></i>
        </div>

        <div>
            <h1>Paiement sécurisé</h1>

            <p>
                Effectuez votre paiement pour accéder au document premium.
            </p>
        </div>

    </div>


    {{-- ============================================================
        ALERTES
    ============================================================= --}}

    @if(session('error'))

        <div class="payment-alert payment-alert-danger">

            <i class="bi bi-exclamation-triangle-fill"></i>

            <span>
                {{ session('error') }}
            </span>

        </div>

    @endif


    @if(session('info'))

        <div class="payment-alert payment-alert-info">

            <i class="bi bi-info-circle-fill"></i>

            <span>
                {{ session('info') }}
            </span>

        </div>

    @endif


    @if($errors->any())

        <div class="payment-alert payment-alert-danger">

            <i class="bi bi-exclamation-triangle-fill"></i>

            <div>

                <strong>
                    Veuillez vérifier les informations.
                </strong>

                <ul>

                    @foreach($errors->all() as $error)

                        <li>
                            {{ $error }}
                        </li>

                    @endforeach

                </ul>

            </div>

        </div>

    @endif


    {{-- ============================================================
        DOCUMENT
    ============================================================= --}}
    <div class="payment-card">

        <div class="document-preview">

            @if($document->cover_image)

                <img
                    src="{{ asset('storage/' . $document->cover_image) }}"
                    alt="{{ $document->title }}"
                >

            @else

                <div class="document-placeholder">

                    <i class="bi bi-file-earmark-pdf-fill"></i>

                </div>

            @endif

        </div>


        <div class="document-info">

            <span class="premium-badge">

                <i class="bi bi-star-fill"></i>

                DOCUMENT PREMIUM

            </span>


            <h2>
                {{ $document->title }}
            </h2>


            @if($document->description)

                <p>
                    {{ $document->description }}
                </p>

            @endif


            <div class="document-meta">

                @if($document->documentType)

                    <span>

                        <i class="bi bi-file-text"></i>

                        {{ $document->documentType->name }}

                    </span>

                @endif


                <span>

                    <i class="bi bi-file-earmark-pdf"></i>

                    PDF

                </span>


                @if($document->file_size)

                    <span>

                        <i class="bi bi-hdd"></i>

                        {{ number_format(
                            $document->file_size / 1024 / 1024,
                            2,
                            ',',
                            ' '
                        ) }}

                        Mo

                    </span>

                @endif

            </div>

        </div>

    </div>


    {{-- ============================================================
        PRIX
    ============================================================= --}}
    <div class="price-section">

        <div>

            <span class="price-label">
                Montant à payer
            </span>

            <span class="price-note">
                Paiement unique
            </span>

        </div>


        <div class="price-value">

            <strong>
                {{ number_format(
                    $document->price,
                    0,
                    ',',
                    ' '
                ) }}
            </strong>

            <span>
                FCFA
            </span>

        </div>

    </div>


    {{-- ============================================================
        PAIEMENT HOSTED LIGDICASH
    ============================================================= --}}
    <div class="form-section">

        <div class="section-title">

            <div class="section-number">
                1
            </div>

            <div>

                <h3>
                    Paiement avec LigdiCash
                </h3>

                <p>
                    Vous serez redirigé vers la plateforme sécurisée
                    LigdiCash pour effectuer votre paiement.
                </p>

            </div>

        </div>


        {{-- ========================================================
            INFORMATIONS
        ========================================================= --}}
        <div class="ligdicash-card">

            <div class="ligdicash-logo">

                <i class="bi bi-wallet2"></i>

            </div>


            <div class="ligdicash-content">

                <strong>
                    Paiement sécurisé par LigdiCash
                </strong>

                <p>
                    Après avoir cliqué sur le bouton ci-dessous,
                    vous serez redirigé vers LigdiCash.
                    Vous pourrez alors sélectionner le moyen de
                    paiement disponible et finaliser votre transaction
                    directement sur leur plateforme.
                </p>


                <div class="ligdicash-features">

                    <span>

                        <i class="bi bi-shield-check"></i>

                        Paiement sécurisé

                    </span>


                    <span>

                        <i class="bi bi-phone"></i>

                        Mobile Money

                    </span>


                    <span>

                        <i class="bi bi-lock"></i>

                        Transaction protégée

                    </span>

                </div>

            </div>

        </div>

    </div>


    {{-- ============================================================
        ÉTAPES
    ============================================================= --}}
    <div class="steps-section">

        <div class="steps-title">

            <i class="bi bi-info-circle"></i>

            <span>
                Comment ça fonctionne ?
            </span>

        </div>


        <div class="payment-steps">

            <div class="payment-step">

                <div class="step-icon">
                    <i class="bi bi-box-arrow-up-right"></i>
                </div>

                <div>

                    <strong>
                        1. Redirection
                    </strong>

                    <span>
                        Vous êtes redirigé vers LigdiCash.
                    </span>

                </div>

            </div>


            <div class="payment-step">

                <div class="step-icon">
                    <i class="bi bi-wallet2"></i>
                </div>

                <div>

                    <strong>
                        2. Paiement
                    </strong>

                    <span>
                        Vous choisissez votre moyen de paiement
                        sur LigdiCash.
                    </span>

                </div>

            </div>


            <div class="payment-step">

                <div class="step-icon">
                    <i class="bi bi-check-circle"></i>
                </div>

                <div>

                    <strong>
                        3. Confirmation
                    </strong>

                    <span>
                        Votre paiement est vérifié automatiquement.
                    </span>

                </div>

            </div>

        </div>

    </div>


    {{-- ============================================================
        SÉCURITÉ
    ============================================================= --}}
    <div class="security-box">

        <i class="bi bi-shield-check"></i>

        <div>

            <strong>
                Accès protégé
            </strong>

            <p>
                Le document ne sera accessible qu'après confirmation
                effective de votre paiement. La confirmation est
                effectuée automatiquement par notre système.
            </p>

        </div>

    </div>


    {{-- ============================================================
        FORMULAIRE
    ============================================================= --}}
    <form
        action="{{ route(
            'payments.store',
            ['document' => $document->id]
        ) }}"
        method="POST"
        id="paymentForm"
    >

        @csrf


        {{-- ========================================================
            BOUTON
        ========================================================= --}}
        <button
            type="submit"
            class="payment-submit"
            id="paymentSubmit"
        >

            <i
                class="bi bi-box-arrow-up-right"
                id="paymentSubmitIcon"
            ></i>

            <span id="paymentSubmitText">
                Continuer vers LigdiCash
            </span>

            <i class="bi bi-arrow-right"></i>

        </button>

    </form>


    {{-- ============================================================
        RETOUR
    ============================================================= --}}
    <div class="payment-footer">

        <a href="{{ url()->previous() }}">

            <i class="bi bi-arrow-left"></i>

            Retour au document

        </a>


        <div class="footer-security">

            <i class="bi bi-shield-lock"></i>

            Paiement sécurisé par LigdiCash

        </div>

    </div>

</div>
```

</div>

<style>

/* ================================================================
   PAGE
================================================================ */

.payment-page{

    min-height:calc(100vh - 80px);

    background:
        radial-gradient(
            circle at top left,
            rgba(37,99,235,.08),
            transparent 35%
        ),
        linear-gradient(
            135deg,
            #f8fafc,
            #eef4ff
        );

    padding:50px 20px;
}


.payment-container{

    width:100%;

    max-width:850px;

    margin:auto;
}


/* ================================================================
   HEADER
================================================================ */

.payment-header{

    display:flex;

    align-items:center;

    gap:18px;

    background:
        linear-gradient(
            135deg,
            #0f3d91,
            #2563eb
        );

    color:#fff;

    padding:28px 32px;

    border-radius:24px 24px 0 0;

    box-shadow:
        0 15px 40px rgba(15,61,145,.18);
}


.payment-security-icon{

    width:55px;

    height:55px;

    display:flex;

    align-items:center;

    justify-content:center;

    border-radius:16px;

    background:rgba(255,255,255,.15);

    font-size:25px;
}


.payment-header h1{

    margin:0;

    font-size:25px;

    font-weight:800;
}


.payment-header p{

    margin:5px 0 0;

    color:#dbeafe;

    font-size:14px;
}


/* ================================================================
   ALERTES
================================================================ */

.payment-alert{

    display:flex;

    align-items:flex-start;

    gap:12px;

    margin-top:18px;

    padding:16px 18px;

    border-radius:14px;

    font-size:14px;
}


.payment-alert-danger{

    background:#fef2f2;

    border:1px solid #fecaca;

    color:#991b1b;
}


.payment-alert-info{

    background:#eff6ff;

    border:1px solid #bfdbfe;

    color:#1e40af;
}


.payment-alert ul{

    margin:8px 0 0;

    padding-left:18px;
}


/* ================================================================
   DOCUMENT
================================================================ */

.payment-card{

    display:flex;

    gap:24px;

    background:#fff;

    padding:28px;

    border:1px solid #e2e8f0;

    border-top:0;

    box-shadow:
        0 15px 40px rgba(15,23,42,.08);
}


.document-preview{

    width:150px;

    min-width:150px;

    height:190px;

    overflow:hidden;

    border-radius:15px;

    background:#f1f5f9;
}


.document-preview img{

    width:100%;

    height:100%;

    object-fit:cover;
}


.document-placeholder{

    width:100%;

    height:100%;

    display:flex;

    align-items:center;

    justify-content:center;

    color:#dc2626;

    font-size:55px;
}


.document-info{

    flex:1;
}


.premium-badge{

    display:inline-flex;

    align-items:center;

    gap:6px;

    padding:6px 11px;

    border-radius:30px;

    background:#fff7ed;

    color:#ea580c;

    font-size:11px;

    font-weight:800;

    letter-spacing:.5px;
}


.document-info h2{

    margin:14px 0 8px;

    font-size:23px;

    line-height:1.3;

    color:#0f172a;
}


.document-info p{

    color:#64748b;

    line-height:1.7;

    margin-bottom:15px;
}


.document-meta{

    display:flex;

    flex-wrap:wrap;

    gap:12px;
}


.document-meta span{

    display:inline-flex;

    align-items:center;

    gap:5px;

    font-size:12px;

    color:#64748b;
}


/* ================================================================
   PRIX
================================================================ */

.price-section{

    display:flex;

    align-items:center;

    justify-content:space-between;

    margin-top:20px;

    padding:24px 28px;

    background:#fff;

    border:1px solid #e2e8f0;

    border-radius:18px;

    box-shadow:
        0 8px 25px rgba(15,23,42,.05);
}


.price-label{

    display:block;

    color:#334155;

    font-weight:700;
}


.price-note{

    display:block;

    margin-top:4px;

    color:#94a3b8;

    font-size:12px;
}


.price-value{

    display:flex;

    align-items:baseline;

    gap:6px;

    color:#1d4ed8;
}


.price-value strong{

    font-size:32px;

    font-weight:900;
}


.price-value span{

    font-size:14px;

    font-weight:700;
}


/* ================================================================
   SECTION
================================================================ */

.form-section{

    margin-top:22px;

    padding:28px;

    background:#fff;

    border:1px solid #e2e8f0;

    border-radius:20px;
}


.section-title{

    display:flex;

    gap:14px;

    margin-bottom:22px;
}


.section-number{

    width:35px;

    height:35px;

    min-width:35px;

    display:flex;

    align-items:center;

    justify-content:center;

    border-radius:10px;

    background:#eff6ff;

    color:#2563eb;

    font-weight:800;
}


.section-title h3{

    margin:0;

    font-size:16px;

    color:#0f172a;
}


.section-title p{

    margin:4px 0 0;

    font-size:13px;

    color:#64748b;

    line-height:1.6;
}


/* ================================================================
   LIGDICASH
================================================================ */

.ligdicash-card{

    display:flex;

    align-items:flex-start;

    gap:18px;

    padding:22px;

    border:1px solid #bfdbfe;

    border-radius:18px;

    background:
        linear-gradient(
            135deg,
            #eff6ff,
            #f8fbff
        );
}


.ligdicash-logo{

    width:58px;

    height:58px;

    min-width:58px;

    display:flex;

    align-items:center;

    justify-content:center;

    border-radius:15px;

    background:
        linear-gradient(
            135deg,
            #1d4ed8,
            #2563eb
        );

    color:#fff;

    font-size:26px;

    box-shadow:
        0 8px 20px rgba(37,99,235,.2);
}


.ligdicash-content{

    flex:1;
}


.ligdicash-content strong{

    display:block;

    margin-bottom:7px;

    color:#0f172a;

    font-size:16px;
}


.ligdicash-content p{

    margin:0;

    color:#475569;

    font-size:13px;

    line-height:1.7;
}


.ligdicash-features{

    display:flex;

    flex-wrap:wrap;

    gap:10px;

    margin-top:15px;
}


.ligdicash-features span{

    display:inline-flex;

    align-items:center;

    gap:6px;

    padding:7px 10px;

    border-radius:8px;

    background:#fff;

    border:1px solid #dbeafe;

    color:#475569;

    font-size:11px;

    font-weight:600;
}


.ligdicash-features i{

    color:#2563eb;
}


/* ================================================================
   ÉTAPES
================================================================ */

.steps-section{

    margin-top:22px;

    padding:24px 28px;

    background:#fff;

    border:1px solid #e2e8f0;

    border-radius:20px;
}


.steps-title{

    display:flex;

    align-items:center;

    gap:8px;

    margin-bottom:18px;

    color:#334155;

    font-size:14px;

    font-weight:800;
}


.steps-title i{

    color:#2563eb;
}


.payment-steps{

    display:grid;

    grid-template-columns:repeat(3,1fr);

    gap:15px;
}


.payment-step{

    display:flex;

    align-items:flex-start;

    gap:10px;

    padding:14px;

    border-radius:13px;

    background:#f8fafc;

    border:1px solid #e2e8f0;
}


.step-icon{

    width:34px;

    height:34px;

    min-width:34px;

    display:flex;

    align-items:center;

    justify-content:center;

    border-radius:9px;

    background:#eff6ff;

    color:#2563eb;
}


.payment-step strong{

    display:block;

    color:#0f172a;

    font-size:12px;

    margin-bottom:3px;
}


.payment-step span{

    display:block;

    color:#64748b;

    font-size:11px;

    line-height:1.5;
}


/* ================================================================
   SÉCURITÉ
================================================================ */

.security-box{

    display:flex;

    gap:14px;

    margin-top:22px;

    padding:18px;

    border-radius:16px;

    background:#f0fdf4;

    border:1px solid #bbf7d0;

    color:#166534;
}


.security-box>i{

    font-size:24px;
}


.security-box strong{

    display:block;

    margin-bottom:4px;
}


.security-box p{

    margin:0;

    font-size:12px;

    line-height:1.6;

    color:#4d7c5a;
}


/* ================================================================
   BOUTON
================================================================ */

.payment-submit{

    width:100%;

    display:flex;

    align-items:center;

    justify-content:center;

    gap:10px;

    margin-top:22px;

    padding:17px 22px;

    border:0;

    border-radius:15px;

    background:
        linear-gradient(
            135deg,
            #ea580c,
            #dc2626
        );

    color:#fff;

    font-size:16px;

    font-weight:800;

    cursor:pointer;

    box-shadow:
        0 12px 30px rgba(220,38,38,.22);

    transition:.25s ease;
}


.payment-submit:hover{

    transform:translateY(-2px);

    box-shadow:
        0 16px 35px rgba(220,38,38,.28);
}


.payment-submit:active{

    transform:scale(.98);
}


.payment-submit:disabled{

    opacity:.7;

    cursor:not-allowed;

    transform:none;
}


.payment-loading-icon{

    animation:paymentSpin 1s linear infinite;
}


@keyframes paymentSpin{

    from{
        transform:rotate(0deg);
    }

    to{
        transform:rotate(360deg);
    }

}


/* ================================================================
   FOOTER
================================================================ */

.payment-footer{

    display:flex;

    align-items:center;

    justify-content:space-between;

    margin-top:20px;

    padding:0 5px;
}


.payment-footer a{

    display:inline-flex;

    align-items:center;

    gap:7px;

    color:#64748b;

    font-size:13px;

    text-decoration:none;
}


.payment-footer a:hover{

    color:#2563eb;
}


.footer-security{

    display:flex;

    align-items:center;

    gap:6px;

    color:#64748b;

    font-size:12px;
}


/* ================================================================
   RESPONSIVE
================================================================ */

@media(max-width:700px){

    .payment-page{

        padding:25px 12px;
    }


    .payment-header{

        padding:22px;

        border-radius:18px 18px 0 0;
    }


    .payment-header h1{

        font-size:20px;
    }


    .payment-card{

        flex-direction:column;

        padding:20px;
    }


    .document-preview{

        width:100%;

        height:230px;
    }


    .price-section{

        padding:20px;
    }


    .price-value strong{

        font-size:25px;
    }


    .form-section{

        padding:20px;
    }


    .ligdicash-card{

        flex-direction:column;
    }


    .payment-steps{

        grid-template-columns:1fr;
    }


    .steps-section{

        padding:20px;
    }


    .payment-footer{

        flex-direction:column;

        gap:12px;

        align-items:flex-start;
    }

}

</style>

<script>

document.addEventListener('DOMContentLoaded', function () {

    const form =
        document.getElementById('paymentForm');

    const button =
        document.getElementById('paymentSubmit');

    const buttonText =
        document.getElementById('paymentSubmitText');

    const buttonIcon =
        document.getElementById('paymentSubmitIcon');


    if (!form || !button || !buttonText) {
        return;
    }


    /*
    |--------------------------------------------------------------------------
    | SOUMISSION
    |--------------------------------------------------------------------------
    |
    | Aucun paiement n'est effectué directement dans le navigateur.
    |
    | Le formulaire est envoyé à Laravel.
    |
    | PaymentController::store()
    |          ↓
    | LigdiCashService::createInvoice()
    |          ↓
    | payment_url
    |          ↓
    | redirect()->away()
    |
    */

    form.addEventListener('submit', function () {

        button.disabled = true;


        if (buttonIcon) {

            buttonIcon.className =
                'bi bi-arrow-repeat payment-loading-icon';

        }


        buttonText.textContent =
            'Redirection vers LigdiCash...';

    });

});

</script>

@endsection
